...
- [Prod Server Monitoring](project_prod_server.md) — SSH/PM2/логи/БД на bestsrv (punchme.oldgod.online). Лайв-мониторинг, deploy.sh pipeline, **Claude CLI auth через env-токен `CLAUDE_CODE_OAUTH_TOKEN` (фикс рецидива разлогина)**, e2e-прогон `npm run e2e`.

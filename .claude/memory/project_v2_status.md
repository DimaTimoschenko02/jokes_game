...
## Сессия 2026-07-17 — HUMAN OPENINGS ЗАДЕПЛОЕНО В ПРОД

- `main` = prod = merge `bad9edb` (dev → main, один revert-коммит). **Откат: `git revert -m 1 bad9edb` на main + push.** Мягкий откат — просто не выбирать режим «Игроки» (дефолт «ИИ» не изменён поведенчески).
- Фича по плану `docs/superpowers/plans/2026-07-11-human-openings.md` (статус в плане обновлён): селект режима при создании комнаты; в human-режиме каждый раунд фаза `opening-writing` 60с (все люди пишут по 1 началу, редактируемо, ранний финиш когда все сдали); AI догенеривает долю ботов/несдавших (резерв префетчится параллельно с写ущими людьми); пул шаффлится, раздача через `buildCircularPromptAssignments(playerIds, openingAuthors)` с constraint «автор не получает своё» (ретрай до 200 шаффлов, n=2 нерешаем — принимаем); автор начала раскрывается в дуэли на тех же условиях что авторы панчлайнов; персист в `prompt_starters` (`source='human'`, `author_user_id`, миграция 0008) → фидбек/голден-цикл работают автоматически. `roundIndex` НЕ инкрементится в opening-writing (клиент показывает roundIndex+1).
- **Попутно пофикшен серьёзный старый баг:** rating ПЕРВОГО раунда каждой игры мгновенно закрывался — клиентский backstop `flushRatings` стрелял на первом рендере rating-фазы со stale `localTimer=0` от reveal-отсчёта, оба клиента слали пустые оценки → фаза закрывалась за <1с. Фикс: `ratingFlushArmedRef` — backstop взводится только после того как таймер rating реально тикал >2с. Верифицировано e2e: r1 ratings 6/6 вместо 0. (Вероятно связан с жалобами на «пропадающие» фазы; баг 3.5 про голосование — всё ещё отдельно и отложен.)
- e2e: `SMOKE_MODE=human` в `play-smoke.mjs` (опенинг-фаза, свой textarea/кнопка). Оба режима прогнаны локально — exit 0, 0 error signals, human-опенинги в БД с автором. Прод после деплоя: API рестартнул, миграция 0008 применена, сайт 200.
- Локальный e2e-стек: порт 5433/11434 занят чужими контейнерами (pgvector/ollama от tg-bot-v2, автостарт Docker Desktop) → punchme-postgres поднимался вручную на 5434 с volume `punchme_punchme-postgres-data`, ollama использовалась общая (bge-m3 там уже есть).

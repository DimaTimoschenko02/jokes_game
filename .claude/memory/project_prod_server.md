...
На prod-сервере под `qwe` **нет глобального `~/.claude/CLAUDE.md`** — соответственно вся CLAUDE.md leak проблема на проде ИРРЕЛЕВАНТНА. Логика [[claude-md-isolation]] (копирование только `.credentials.json` + `settings.json` в `$TEMP/punchme-claude-home`) — реально нужна только для локалки dev. На сервере она работает no-op (нечего исключать). Можно потенциально упростить/закомментить часть isolated HOME логики если станет в тягость, но сейчас она безвредна.

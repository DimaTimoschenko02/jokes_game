export const ROUND_COUNT_DEFAULT: number = 4
export const ROUND_COUNT_MIN: number = 2
export const ROUND_VOTE_WEIGHTS: readonly number[] = [100, 150, 200, 300]
export const ROUND_COUNT_MAX: number = 4
export const BOT_COUNT_MIN: number = 1
export const BOT_COUNT_MAX: number = 2
export const WRITING_PHASE_SECONDS: number = 180
export const OPENING_WRITING_PHASE_SECONDS: number = 60
export const OPENING_MIN_LENGTH: number = 6
export const OPENING_MAX_LENGTH: number = 200
export const VOTING_PHASE_SECONDS: number = 35
export const VOTING_REVEAL_SECONDS: number = 6
export const RATING_PHASE_SECONDS: number = 30
export const PLAYER_NAME_MAX_LENGTH: number = 30
export const ANSWER_MAX_LENGTH: number = 140
